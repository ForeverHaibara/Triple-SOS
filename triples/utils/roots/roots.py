...
def _algebraic_extension(vec, domain):
    if len(vec) == 0:
    ...
    if domain.is_QQ or domain.is_ZZ or (not hasattr(domain, 'mod')):
        ...
        #     vecs.append([x.rep[-i] if len(x.rep) >= i else zero for x in vec])
...
class Root():
    ...
    def to_sympy(self, *args, **kwargs) -> sp.Expr:
    ...
    def subs(self, expr: Union[sp.Basic], symbols: Optional[List[sp.Symbol]] = None) -> Union[sp.Basic]:
        """
    ...
    def span(self, n: int, diff: Optional[Tuple[int, ...]] = None, **options) -> sp.Matrix:
        ...
        # if not self.is_algebraic:
        #     return self.as_vec(n, diff, **options)
    ...
    def _subs_poly_rep(self, poly: sp.Poly) -> ANP:
        """
        ...
        if not (self.domain.is_QQ or self.domain.is_ZZ
            ...
            if self.domain != poly.domain:
                # TODO: shall we use low-level conversion?
                self = Root(self.root, domain=poly.domain)
        s = poly.domain.zero
...
#     """
#     __slots__ = ('root', 'K', 'rep', '_inv_sum', '_power_cache')
...
#             self.domain = sp.QQ
#             for r in root:
#                 if not (isinstance(r, sp.Rational) or r in self.domain):
#                     self.domain = self.domain.algebraic_field(r)
#             self.rep = [
#                self.domain.from_sympy(r) for r in root
#             ]
#         if self.rep is None:
#             self.rep = self.root
    
#         if self.domain is None:
#             self._inv_sum = sp.S(1) / sum(self.rep)
#         else:
#             _sum = sum(self.rep)
#             if _sum != self.domain.zero:            
#                 self._inv_sum = self.domain.from_sympy(sp.S(1)) / sum(self.rep)
#             else:
#                 self._inv_sum = sp.oo
...
#         if k is None:
#             if i >= 0:
#                 self._power_cache[i][degree] = self.rep[i] ** degree
#             elif i == -1:
...
#         if to_sympy:
#             results = [self.domain.to_sympy(r) for r in results]
#         if is_single_expr:
#             return results[0]
#         if not to_sympy:
...
#         uv = self._uv
#         if hasattr(self, 'K') and self.domain is not None and to_sympy:
#             return (self.domain.to_sympy(uv[0]), self.domain.to_sympy(uv[1]))
#         return uv
...
#         if self.domain is not None and to_sympy:
