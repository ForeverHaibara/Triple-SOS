...
def _save_batch(result, dir_path, bench_id):
    file = os.path.join(dir_path, f'results-{bench_id}.jsonl')
...
def run_bench(benchmarks=BENCHMARKS, save_interval=0):
    ...
    temp_dir = f"./.benchmarks/bench-{bench_id}"
        ...
        for i, (problem_name, problem) in enumerate(problems.items(), start=1):
            ...
            if mark.skip in problem_marks:
                ...
                if (end_time - last_save_time).total_seconds() >= save_interval:
                    _save_batch(batch_results, temp_dir, bench_id)
    ...
    with open(f"{temp_dir}/bench-{bench_id}.json", 'w') as f:
        json.dump(header, f, indent=4)
    print(f"Benchmark results saved to {temp_dir}/bench-{bench_id}.json")
