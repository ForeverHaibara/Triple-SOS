...
def show_dets(M: Matrix):
    ...
    syms = dict(zip(syms, [Symbol('x'), Symbol('y')]))
    det = []
    for i in range(1, 1+(M.shape[0])):
        value = (GreaterThan(M[:i,:i].det().factor().subs(syms), 0))
        if value is sympy_true:
            value = Integer(1)
        det.append(value)
    txt = wrap_desmos(det)
    display(HTML(txt))
...
def plot_contour(
    ...
    f_lambdified = lambdify(vars, f, "numpy")
    x_values = linspace(*xrange, 400)
    y_values = linspace(*yrange, 400)
