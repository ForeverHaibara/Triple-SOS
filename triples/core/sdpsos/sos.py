...
from sympy import Poly, Expr, Symbol, ZZ
...
class SOSProblem():
    ...
    def dof(self) -> int:
        """
        Return the degree of freedom of the child node of the SDP problem.
        """
    ...
    def S_from_y(self, y: Optional[Union[Matrix, ndarray, Dict]] = None) -> Matrix:
        """
    ...
    def as_solution(
        ...
        eqspace = {eq: x + space * self._sdp.y for eq, (x, space) in self._eq_space.items()}
    ...
    def construct(
        ...
        ###################################################################
        if degree is None:
            # TODO: what if the largest degree is odd?
        ...
        sdp, eq_space = SDPProblem.from_entry_contribution(
            monomial_manager.arraylize_sp(poly),
