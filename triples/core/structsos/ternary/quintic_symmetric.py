...
def sos_struct_quintic_symmetric(coeff: Coeff, real = True):
    ...
    Proof:
    ```
    F(a,b,c)s((a-b)^2) = s(c(2c^3-2abc+(t-1)(a^2c+b^2c-a^2b-ab^2)-(t+1)(ac^2+bc^2-2abc))^2)
    ...
    Proof: Denote
    ```
    g(a,b,c) = ((3-u)/(4u)(a+b) - c)(a^2+b^2+c^2 - (t+1)/2c(a+b) + (t-2)ab) 
        - 9(u-1)^2(u+1)(u+3)/(32u^3)*ab(2c-a-b).
    ```
    Then,
    ```
    F(a,b,c) * s(a^2 - (7u^2+30u-9)/(16u^2)ab) 
        = CyclicSum(cg(a,b,c)^2) + 27(u-3)(u^2-u+2)/(32u^3)*CyclicSum((a-b)^2(a+b-tc)^2) >= 0.
    ```
...
def _sos_struct_quintic_symmetric_trivial(coeff: Coeff):
    ...
    det1 = coeff.from_list([16, -32, -16*u - 8*v - 32*z,
            -16*u + 8*v - 16, 4*u**2 + 4*u*v + 8*u + v**2 + 8*v*z + 4*v + 4], gens=(a,)).as_poly()
