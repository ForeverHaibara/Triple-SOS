...
class SDPProblem(SDPTransformMixin):
    ...
    def __init__(
        ...
        if len(keys):
            ...
            if free_symbols is not None:
                ...
                self.free_symbols = list(Symbol('y_{%d}'%i) for i in range(dof))
        else:
            self.free_symbols = []
    ...
    def _construct_sdp(self,
            reg: float = 0,
            constraints: List[Union[PicosExpression, Expr, Callable]] = []
        ):
        """
    ...
    def _align_objective(
            self,
            objective: Tuple[str, Union[PicosExpression, Expr, Callable]]
    ...
    def _align_constraint(
        self,
        constraint: Union[PicosExpression, Expr, Callable]
    ) -> PicosExpression:
        """
    ...
    def _nsolve_with_obj(
            self,
            objectives: List[Tuple[str, Union[PicosExpression, Expr, Callable]]],
    ...
    def _solve_trivial(
            self,
            objectives: Optional[List[Tuple[str, Union[PicosExpression, Expr, Callable]]]] = None,
            constraints: List[Union[PicosExpression, Expr, Callable]] = []
        ) -> Optional[Tuple[Matrix, Decomp]]:
        """
    ...
    def _solve_degenerated(
        ...
        if self.dof == 0:
            decomp = rationalize_and_decompose(
                Matrix([]).reshape(0,1), self._x0_and_space,
                check_pretty = False
            )
            return decomp
