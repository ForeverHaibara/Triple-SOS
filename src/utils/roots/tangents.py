...
class RootTangent():
    ...
    def expr(self):
        if self.expr_ is None:
            self.expr_ = preprocess_text(self.s)
        return self.expr_
...
def root_tangents(rootsinfo, tol=1e-6, rounding=1e-5):
    """
    ...
    """
    roots, strict_roots = rootsinfo.roots, rootsinfo.strict_roots
    ...
    for root in roots:
        ...
        if b == 0:
            ...
            # a = rationalize(a, rounding=1e-3)
            # b = rationalize(b, rounding=1e-3)
            # if b == 0 and a > 0 and a != 1 and a.p < 50 and a.q < 50:
            #     tangents.append(RootTangent(f'{a}*(a-b)+b-c', coor = root))
...
def _root_tangents_quadratic(a, b, rounding = 1e-3, strict = False, uncentered = False):
    ...
    frac = lambda x: rationalize(x, rounding = rounding)
    tangents = []
