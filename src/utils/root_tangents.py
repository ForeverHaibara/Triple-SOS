...
def _root_tangents_quadratic(a, b, rounding = 1e-3, uncentered = False):
    """
    ...
    u, v = frac(u), frac(v)
    tangents.append(f'a2-b2+{u}(ab-ac)+{v}(bc-ab)')
    ...
    if abs(u) > 1e-3:
        v = frac((a*a + b*b + 1) / u)
        if v != 1 and v > 1e-3:
            tangents.append(f'a2+b2+c2-{v}*(ab+bc+ca)')
...
def _root_tangents_cubic(a, b, rounding = 1e-3, uncentered = False):
    ...
    if abs(a) > 1e-3 and abs(b) > 1e-3:
        ...
        v = frac(v)
        if v != 1 and v > 1e-3:
            tangents.append(f'a2b+b2c+c2a-{v}abc')
            
        v = ((b*b + a)*a + b) / u
        v = frac(v)
        if v != 1 and v > 1e-3:
            tangents.append(f'ab2+bc2+ca2-{v}abc')
    ...
    if isinstance(a, sp.Float):
        if a < 1e-3:
            ...
            if a_.q < 100 or abs(a_.p) < 100:
                # (1/a)*a + (1-1/a)*b - c
                if a_ > 1:
                    tangents.append(f'{1/a_}a+{1-1/a_}b-c')
                else:
                    tangents.append(f'{1/a_}a-{1/a_-1}b-c')
